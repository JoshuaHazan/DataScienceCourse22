...
library(cluster)
set.seed(302)
#Best k-value for clustering with untransformed data
Ks=sapply(2:7,
function(i)
...
#Best k-value for clustering with transformed data
Ks=sapply(2:7,
function(i)
...
#The highest value is obtained for k=4 for both untransformed and transformed
pamcluster_raw=cluster::pam(t(mat),k=4)
plot(silhouette(pamcluster_raw),main=NULL)

pamcluster_scaled=cluster::pam(t(scaledMat),k=4)
plot(silhouette(pamcluster_scaled),main=NULL)
```
...
tsne_p10 <- Rtsne(t(scaledMat),perplexity = 10)
plot(tsne_p10$Y,col=as.factor(annotation_col$LeukemiaType),
     pch=19)
legend("bottomright",
legend=unique(annotation_col$LeukemiaType),
fill =palette("default"),
...
tsne_p3 <- Rtsne(t(scaledMat),perplexity = 3)
plot(tsne_p3$Y,col=as.factor(annotation_col$LeukemiaType),
     pch=19)
legend("bottomright",
legend=unique(annotation_col$LeukemiaType),
fill =palette("default"),
...
tsne_p18 <- Rtsne(t(scaledMat),perplexity = 18)
plot(tsne_p18$Y,col=as.factor(annotation_col$LeukemiaType),
     pch=19)
legend("topright",
legend=unique(annotation_col$LeukemiaType),
fill =palette("default"),
...
#Changing the perplexity changes how the clusters are spread out. 
#At very low perplexity, very small sub-clusters form.
#At very high perplexity, clusters do not form properly. 
```
